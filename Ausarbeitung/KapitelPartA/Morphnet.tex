...
Bei einem residualen Netzwerk kann weiterhin ein ganzer Block wegfallen. In diesem Fall müssen die Kanal-Union-Listen angepasst werden (und es wird mit einem ohne diesen Block um mehrere Layer verkürztes Netzwerk weitergemacht)\todo[color=blue1]{wie wäre es mit: ..., die weitere Aktion wird ohne diesen Block im um mehrere Layer verkürzten Netzwerk weiter geführt?}.
...
\begin{equation}
 ...
 \end{cases} 
 \end{equation}
 Mit Hilfe dieser Mapping-Funktion werden nun die neuen Gewichtstensoren initialisiert:\todo[inline,color=red]{Was ist an dieser Formel nicht in Ordnung?}
 \begin{align*}
...
Die manuelle Wahl von Hyperparametern, die bestimmen wie groß und komplex ein neuronales Netz ist, braucht eine gewisse Kunstfertigkeit. Sind die Hyperparameter falsch gewählt, so müssen diese angepasst und das Netz erneut trainiert werden\todo[color=blue1]{das ist eine Binsenweisheit... welche Information wolltest Du rüberbringen? 'Bei der Wahl des Gemüses für das Rezept ist eine gewisse Kunstfertigkeit gefragt - falls man das falsche erwischt hat, muss man dies korrigieren:-)'}. Mit Hilfe der hier vorgestellten Methode wird die Suche nach der besten Architektur automatisiert. Dies geschieht mit Hilfe von iterativen Verkleinern und Vergrößern des Netzes. Diese Methode hat drei Vorteile:
\begin{enumerate}
